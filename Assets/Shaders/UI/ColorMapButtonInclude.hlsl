...
// Texturing.
...
#endif
#else
    float2 myTransferUv = input.uv;
    
    myTransferUv.y = 0;
    //myTransferUv.y *= _colorMapParams.z; //clamp to size of single line
    //myTransferUv.y += (_colorMapParams.x * 0.5); //center in line
    myTransferUv.y += _colorMapParams.z * _colorMapParams.x * 0.5; //center in line
    myTransferUv.y += _colorMapParams.x * _colorMapParams.y * _colorMapParams.z;
    ...
    float2 dx = ddx(myTransferUv) * 0.25; // horizontal offset
    float2 dy = ddy(myTransferUv) * 0.25; // vertical offset
    // supersampled 2x2 ordered grid
    half4 albedo = half4(0.0h, 0.0h, 0.0h, 0.0h);
#if defined(_URP)
    albedo += SAMPLE_TEXTURE2D_BIAS(_MainTex, sampler_MainTex, float2(myTransferUv + dx + dy), _MipmapBias);
    albedo += SAMPLE_TEXTURE2D_BIAS(_MainTex, sampler_MainTex, float2(myTransferUv - dx + dy), _MipmapBias);
    albedo += SAMPLE_TEXTURE2D_BIAS(_MainTex, sampler_MainTex, float2(myTransferUv + dx - dy), _MipmapBias);
    albedo += SAMPLE_TEXTURE2D_BIAS(_MainTex, sampler_MainTex, float2(myTransferUv - dx - dy), _MipmapBias);
#else
    albedo += tex2Dbias(_MainTex, float4(myTransferUv + dx + dy, 0.0, _MipmapBias));
    albedo += tex2Dbias(_MainTex, float4(myTransferUv - dx + dy, 0.0, _MipmapBias));
    albedo += tex2Dbias(_MainTex, float4(myTransferUv + dx - dy, 0.0, _MipmapBias));
    albedo += tex2Dbias(_MainTex, float4(myTransferUv - dx - dy, 0.0, _MipmapBias));
#endif
...
#else
#if defined(_URP)
    half4 albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, myTransferUv);
#else
    half4 albedo = tex2D(_MainTex, myTransferUv);
#endif
#endif
#endif
#endif
